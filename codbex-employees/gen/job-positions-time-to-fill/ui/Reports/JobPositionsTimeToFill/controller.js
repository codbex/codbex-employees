angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', 'Extensions', function ($scope, messageHub, entityApi, Extensions) {
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("JobPositionsTimeToFill", `Unable to count JobPositionsTimeToFill: '${response.message}'`);
					return;
				}
				if (response.data) {
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("JobPositionsTimeToFill-Report-details", {
				action: "select",
				entity: entity,
			});
		};
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("JobPositionsTimeToFill-Report-filter", {
