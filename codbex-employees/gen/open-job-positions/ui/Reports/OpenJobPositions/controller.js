angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions) => {
		...
		Extensions.getWindows(['codbex-employees-custom-action']).then((response) => {
			$scope.pageActions = response.data.filter(e => e.perspective === 'Reports' && e.view === 'OpenJobPositions' && (e.type === 'page' || e.type === undefined));
			$scope.entityActions = response.data.filter(e => e.perspective === 'Reports' && e.view === 'OpenJobPositions' && e.type === 'entity');
		});
		...
		//-----------------Events-------------------//
		Dialogs.addMessageListener({ topic: 'codbex-employees.Reports.OpenJobPositions.entitySearch', handler: (data) => {
			resetPagination();
		...
		$scope.loadPage = (pageNumber, filter) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				Dialogs.showAlert({
					title: 'OpenJobPositions',
					message: `Unable to count OpenJobPositions: '${message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
