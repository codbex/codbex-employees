...
import { GenderRepository } from '../../data/Settings/GenderRepository'
import { GenderEntity } from '../../data/Settings/GenderEntity'

const validationModules = await Extensions.loadExtensionModules('codbex-employees-Settings-Gender', ['validate']);
...
class GenderController {
    ...
    @Get('/')
    @Documentation('Get All Gender')
    public getAll(_: any, ctx: any): GenderEntity[] {
        try {
    ...
    public create(entity: GenderEntity): GenderEntity {
        try {
            this.checkPermissions('write');
            this.validateEntity(entity);
            entity.Id = this.repository.create(entity) as any;
            response.setHeader('Content-Location', '/services/ts/codbex-employees/gen/codbex-employees/api/Settings/GenderController.ts/' + entity.Id);
    ...
    public search(filter: any): GenderEntity[] {
        try {
            this.checkPermissions('read');
    ...
    public getById(_: any, ctx: any): GenderEntity {
        try {
            this.checkPermissions('read');
    ...
    public update(entity: GenderEntity, ctx: any): GenderEntity {
        try {
            this.checkPermissions('write');
            const id = parseInt(ctx.pathParameters.id);
            entity.Id = id;
    ...
    private checkPermissions(operationType: string) {
        if (operationType === 'read' && !(user.isInRole('codbex-employees.EmployeesSettings.GenderReadOnly') || user.isInRole('codbex-employees.EmployeesSettings.GenderFullAccess'))) {
            throw new ForbiddenError();
        }
        if (operationType === 'write' && !user.isInRole('codbex-employees.EmployeesSettings.GenderFullAccess')) {
            throw new ForbiddenError();
        }
    }
